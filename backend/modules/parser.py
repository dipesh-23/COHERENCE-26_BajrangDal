...
def load_nlp_pipeline() -> spacy.language.Language:
    ...
    """
    # 1. Try to load the large biomedical SciSpaCy model; fall back gracefully
    try:
        nlp = spacy.load("en_core_sci_lg")
    except OSError:
        # Fallback to standard spaCy English models, then to a blank pipeline
    ...
    # 2. Add MedSpaCy components (order matters) when available
...
# Global singleton to prevent reloading ScispaCy on every API call
_global_nlp = None

def get_parser_pipeline():
    global _global_nlp
    if _global_nlp is None:
        _global_nlp = load_nlp_pipeline()
    return _global_nlp
