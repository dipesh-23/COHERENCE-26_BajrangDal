...
Dependencies
------------
    pip install presidio-analyzer presidio-anonymizer pdfplumber httpx
...
def anonymize_patient(patient: dict[str, Any]) -> dict[str, Any]:
    ...
    # If Presidio is not available, return patient unchanged (privacy pipeline disabled)
    if not PRESIDIO_AVAILABLE or _analyzer is None or _anonymizer is None:
        return copy.deepcopy(patient)
